...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.patch('/api/equipment/:id/status', isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.patch('/api/reservations/:id/approve', isAuthenticated, async (req: any, res) => {
    try {
      const user = req.user.claims;
  ...
  app.get('/api/reservations/pending', isAuthenticated, async (req: any, res) => {
    try {
      const user = req.user.claims;
      if (user.role === 'student') {
        return res.status(403).json({ message: "Insufficient permissions" });
  ...
  // Usage log routes
  app.post('/api/usage-logs', isAuthenticated, async (req: any, res) => {
    try {
  ...
  app.get('/api/equipment/:id/usage-logs', isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.get('/api/analytics/equipment-utilization', isAuthenticated, async (req, res) => {
    try {
